...
public static class HtmlVisualizerGenerator
{
    public static string Generate(AtlasResource resource)
    {
        ...
        // Software Bill of Materials (SBOM) & OSV.dev / deps.dev Security Audit
        if (spec.Dependencies?.Sbom?.Components != null && spec.Dependencies.Sbom.Components.Count > 0)
        {
            var components = spec.Dependencies.Sbom.Components;
            var vulns = spec.Dependencies.Sbom.Vulnerabilities ?? new List<CycloneDxVulnerability>();
            var critCount = vulns.Count(v => v.Ratings.Any(r => r.Severity.Equals("Critical", StringComparison.OrdinalIgnoreCase)));
            var highCount = vulns.Count(v => v.Ratings.Any(r => r.Severity.Equals("High", StringComparison.OrdinalIgnoreCase)));

            sb.AppendLine("    <section class=\"card full-card\">");
            sb.AppendLine("      <div class=\"card-header\">");
            sb.AppendLine($"        <h2>🛡️ Software Bill of Materials (SBOM) & Security Audit ({components.Count} Libraries)</h2>");
            if (vulns.Count == 0)
            {
                sb.AppendLine($"        <span class=\"badge sev-badge sev-low\">🟢 {components.Count}/{components.Count} Packages Verified Clean (OSV.dev)</span>");
            }
            else
            {
                sb.AppendLine($"        <span class=\"badge sev-badge sev-critical\">🚨 {vulns.Count} CVEs Found ({critCount} Critical, {highCount} High)</span>");
            }
            sb.AppendLine("      </div>");
            sb.AppendLine("      <p style=\"color: var(--text-secondary); margin-bottom: 1rem; font-size: 0.88rem;\">");
            sb.AppendLine("        Verified against Google <strong>OSV.dev</strong> and GitHub Advisory Database (GHSA) with SPDX license and version lifecycle intelligence from <strong>deps.dev</strong>.");
            sb.AppendLine("      </p>");

            sb.AppendLine("      <div class=\"table-responsive\">");
            sb.AppendLine("        <table class=\"data-table\">");
            sb.AppendLine("          <thead><tr><th>Component</th><th>Version & PURL</th><th>SPDX License</th><th>Security Status (OSV.dev)</th><th>Supply Chain Lifecycle</th></tr></thead>");
            sb.AppendLine("          <tbody>");

            foreach (var comp in components)
            {
                var licName = comp.Licenses.FirstOrDefault()?.License?.Name ?? comp.Licenses.FirstOrDefault()?.License?.Id ?? "MIT";
                var osvUrl = comp.Properties.FirstOrDefault(p => p.Name == "atlas:osv:url")?.Value ?? $"https://osv.dev/list?q={HttpUtility.UrlEncode(comp.Name)}";
                ...
                var compVulns = vulns.Where(v => v.Affects.Any(a => a.Ref == comp.Purl || a.Ref.Contains(comp.Name))).ToList();

                sb.AppendLine("            <tr>");
                sb.AppendLine($"              <td><strong>{HttpUtility.HtmlEncode(comp.Name)}</strong><br><small style=\"color:var(--text-muted);\">{HttpUtility.HtmlEncode(comp.Description)}</small></td>");
                sb.AppendLine($"              <td><code>{HttpUtility.HtmlEncode(comp.Version)}</code><br><small style=\"color:var(--accent-blue);\">{HttpUtility.HtmlEncode(comp.Purl)}</small></td>");
                sb.AppendLine($"              <td><span class=\"tech-tag\" style=\"font-size:0.75rem;\">📄 {HttpUtility.HtmlEncode(licName)}</span></td>");
                
                if (compVulns.Count > 0)
                {
                    sb.AppendLine("              <td>");
                    foreach (var v in compVulns)
                    {
                        var sev = v.Ratings.FirstOrDefault()?.Severity ?? "High";
                        sb.AppendLine($"                <a href=\"{HttpUtility.HtmlEncode(v.Source.Url)}\" target=\"_blank\" class=\"badge sev-badge sev-critical\" style=\"text-decoration:none;\">🔴 {HttpUtility.HtmlEncode(v.Id)} ({HttpUtility.HtmlEncode(sev)})</a><br>");
                    }
                    sb.AppendLine("              </td>");
                }
                else
                {
                    sb.AppendLine($"              <td><a href=\"{HttpUtility.HtmlEncode(osvUrl)}\" target=\"_blank\" class=\"badge sev-badge sev-low\" style=\"text-decoration:none;\">🟢 Verified Clean</a></td>");
                }

                sb.AppendLine("              <td>");
                if (isDep)
                {
                    sb.AppendLine($"                <span class=\"badge sev-badge sev-critical\">🚨 Deprecated</span>{(string.IsNullOrWhiteSpace(depReason) ? "" : $"<br><small style=\"color:var(--text-secondary);\">{HttpUtility.HtmlEncode(depReason)}</small>")}");
                }
                else if (isOutdated && !string.IsNullOrWhiteSpace(latestVer))
                {
                    sb.AppendLine($"                <span class=\"badge sev-badge sev-medium\" style=\"background:#d97706;\">Update: {HttpUtility.HtmlEncode(latestVer)}</span>");
                }
                else
                {
                    sb.AppendLine("                <span style=\"color:#10b981; font-size:0.8rem;\">✓ Up to date</span>");
                }
                sb.AppendLine("              </td>");
        ...
        sb.AppendLine($"    <p>Generated by <strong>Atlas CLI</strong> • {DateTime.UtcNow:yyyy-MM-dd HH:mm:ss} UTC</p>");
