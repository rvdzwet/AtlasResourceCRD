...
public sealed class RepoScanner
{
    ...
    {
        var highValue = new System.Collections.Concurrent.ConcurrentBag<ScannedSourceFile>();
        var maxBytes = maxFileSizeKb * 1024;
        var eligibleFiles = files.Where(f => f.SizeBytes <= maxBytes).ToList();
        var totalEligible = eligibleFiles.Count;

        var processedCount = 0;
        var lastLogTime = DateTime.UtcNow;
        var stopwatch = System.Diagnostics.Stopwatch.StartNew();

        _logger.LogInformation("[RepoScanner] Loading content for {Count} candidate source files (Parallel degree: 32)...", totalEligible);

        System.Threading.Tasks.Parallel.ForEach(eligibleFiles, new System.Threading.Tasks.ParallelOptions { MaxDegreeOfParallelism = 32 }, file =>
        {
            ...
            if (current % 1000 == 0 || (DateTime.UtcNow - lastLogTime).TotalSeconds >= 10.0 || current == totalEligible)
            {
                lock (this)
                {
                    if ((DateTime.UtcNow - lastLogTime).TotalSeconds >= 5.0 || current == totalEligible)
                    {
                        lastLogTime = DateTime.UtcNow;
                        var pct = (double)current / totalEligible * 100.0;
                        _logger.LogInformation("[RepoScanner] Loading progress: {Current}/{Total} files ({Pct:F1}%) in {ElapsedSec:F1}s",
                            current, totalEligible, pct, stopwatch.Elapsed.TotalSeconds);
                    }
                }
            }
        });
        ...
        stopwatch.Stop();
        _logger.LogInformation("[RepoScanner] Finished loading {Count} source files in {ElapsedSec:F2}s.", highValue.Count, stopwatch.Elapsed.TotalSeconds);
    ...
    private static string? CategorizeFile(string rel, string fileName)
    {
        ...
        // Database procedures and SQL scripts (.sp, .proc, .sql, .pls, etc.)
        var ext = Path.GetExtension(lowerName);
        if (ext is ".sp" or ".proc" or ".prc" or ".pls" or ".plsql" or ".pks" or ".pkb" or ".fnc" or ".trg" or ".sql" or ".ddl" or ".dml" or ".tab" or ".vw")
            return "DatabaseProcedure";

        // All non-ignored files are included as source files at all times
        return "Source";
    }
}
