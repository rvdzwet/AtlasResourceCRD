...
public sealed class ManifestAnalyzer
{
    ...
    public ScannedManifest? Analyze(string relativePath, string fullPath)
    {
        ...
        if (!IsManifestCandidate(fileName, extension))
        {
            return null;
        }
    ...
    private static bool IsManifestCandidate(string fileName, string extension)
    {
        if (extension is ".csproj" or ".fsproj" or ".vbproj") return true;
        if (fileName.Equals("package.json", StringComparison.OrdinalIgnoreCase)) return true;
        if (fileName.Equals("requirements.txt", StringComparison.OrdinalIgnoreCase) || fileName.Equals("Pipfile", StringComparison.OrdinalIgnoreCase)) return true;
        if (fileName.Equals("pyproject.toml", StringComparison.OrdinalIgnoreCase)) return true;
        if (fileName.Equals("go.mod", StringComparison.OrdinalIgnoreCase)) return true;
        if (fileName.Equals("Cargo.toml", StringComparison.OrdinalIgnoreCase)) return true;
        if (fileName.StartsWith("Dockerfile", StringComparison.OrdinalIgnoreCase)) return true;
        if (fileName.StartsWith("docker-compose", StringComparison.OrdinalIgnoreCase)) return true;
        return false;
    }
    ...
    private ScannedManifest AnalyzePackageJson(string relativePath, string content)
    {
        ...
        try
        {
            ...
            if (root.TryGetProperty("dependencies", out var deps) && deps.ValueKind == JsonValueKind.Object)
            {
                foreach (var prop in deps.EnumerateObject())
                {
                    manifest.ExtractedPackages.Add(new PackageDependency
                    {
                        ...
                        Purpose = "Runtime NPM dependency"
                    });
            ...
            if (root.TryGetProperty("devDependencies", out var devDeps) && devDeps.ValueKind == JsonValueKind.Object)
            {
                ...
                {
                    if (!manifest.ExtractedPackages.Any(p => string.Equals(p.Name, prop.Name, StringComparison.OrdinalIgnoreCase)))
                    {
                        manifest.ExtractedPackages.Add(new PackageDependency
                        {
                            Name = prop.Name,
                            Version = prop.Value.GetString(),
                            Purpose = "Dev & Tooling dependency"
                        });
                        _logger.LogTrace("[ManifestAnalyzer] Extracted NPM devDependency: {Name} ({Version})", prop.Name, prop.Value.GetString());
                    }
                }
            }
        }
